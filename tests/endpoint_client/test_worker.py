...
class TestWorkerIntegration:
    """Integration tests for Worker with real HTTP requests to echo server."""
    ...
    @pytest.mark.asyncio
    async def test_worker_error_handling(self, worker_config, zmq_config):
        ...
        try:
            ...
            response_data = await asyncio.wait_for(response_pull.recv(), timeout=5.0)
            response = pickle.loads(response_data)
        ...
        try:
            ...
            for _ in range(num_queries):
                response_data = await asyncio.wait_for(
                    response_pull.recv(), timeout=5.0
                )
                response = pickle.loads(response_data)
    ...
    @pytest.mark.asyncio
    async def test_worker_streaming_http_error_handling(
...
class TestWorkerEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_worker_process_request_exception_handling(self, basic_config):
        """Test worker handles exceptions in _process_request gracefully."""
        ...
        try:
            ...
            timeout = aiohttp.ClientTimeout(total=aiohttp_config.client_timeout_total)
            worker._session = aiohttp.ClientSession(timeout=timeout)

            # Create response pull socket to receive error
            response_pull = context.socket(zmq.PULL)
            response_pull.bind(zmq_config.zmq_response_queue_addr)

            # Create a malformed query that will cause an exception
            class BadQuery:
                def __init__(self):
                    self.id = "test-bad-query"
                    self.stream = False
            ...
            # Process the bad query
            await worker._process_request(bad_query)
            ...
            assert isinstance(response, QueryResult)
            assert response.query_id == "test-bad-query"
            assert response.error is not None
            assert "Malformed query" in response.error
    ...
    # Removed redundant test - connection errors are already covered by test_worker_non_streaming_connection_error

    # Removed redundant test - timeout handling is covered by other connection error tests
    ...
    # Removed redundant test - streaming timeout is covered by test_worker_streaming_http_error_handling
...
class TestWorkerHTTPErrorHandling:
    """Test Worker HTTP error handling scenarios."""
    ...
    def error_config(self):
        ...
        timestamp = int(time.time() * 1000)
        http_config = HTTPClientConfig(
            endpoint_url="http://localhost:99999/error",  # Will be overridden per test
            num_workers=1,
        )
        aiohttp_config = AioHttpConfig(client_timeout_total=2.0)
        zmq_config = ZMQConfig(
            zmq_request_queue_prefix=f"ipc:///tmp/test_http_error_{timestamp}",
            zmq_response_queue_addr=f"ipc:///tmp/test_http_error_resp_{timestamp}",
        )
        return http_config, aiohttp_config, zmq_config

    @pytest.mark.asyncio
    async def test_worker_streaming_http_404_error(
        self, mock_http_echo_server, error_config
    ):
        """Test worker handling HTTP 404 error in streaming request."""
        http_config, aiohttp_config, zmq_config = error_config
...
class TestWorkerStreamingJSONParsing:
    """Test Worker JSON parsing in streaming responses."""

    @pytest.fixture
    def streaming_config(self):
        """Create configuration for streaming tests."""
        timestamp = int(time.time() * 1000)
        http_config = HTTPClientConfig(
            endpoint_url="http://localhost:99999/streaming",
            num_workers=1,
        )
        aiohttp_config = AioHttpConfig(client_timeout_total=2.0)
        zmq_config = ZMQConfig(
            zmq_request_queue_prefix=f"ipc:///tmp/test_streaming_{timestamp}",
            zmq_response_queue_addr=f"ipc:///tmp/test_streaming_resp_{timestamp}",
        )
        return http_config, aiohttp_config, zmq_config

    @pytest.mark.asyncio
    async def test_worker_streaming_malformed_json(self, streaming_config):
        """Test worker handling malformed JSON in streaming response."""
        http_config, aiohttp_config, zmq_config = streaming_config
...
class TestWorkerResourceInitialization:
    """Test Worker resource initialization edge cases."""

    @pytest.fixture
    def init_config(self):
        """Create configuration for initialization tests."""
        timestamp = int(time.time() * 1000)
        http_config = HTTPClientConfig(
            endpoint_url="http://localhost:99999/init",
            num_workers=1,
        )
        aiohttp_config = AioHttpConfig()
        zmq_config = ZMQConfig(
            zmq_request_queue_prefix=f"ipc:///tmp/test_init_{timestamp}",
            zmq_response_queue_addr=f"ipc:///tmp/test_init_resp_{timestamp}",
        )
        return http_config, aiohttp_config, zmq_config

    @pytest.mark.asyncio
    async def test_worker_zmq_socket_binding_error(self, init_config):
        """Test worker handling ZMQ socket binding errors."""
        http_config, aiohttp_config, zmq_config = init_config
    ...
    @pytest.mark.asyncio
    async def test_worker_aiohttp_session_creation(self, init_config):
